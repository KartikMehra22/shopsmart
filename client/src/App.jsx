  ...
  return (
    <nav className={`nav ${scrolled ? 'nav--scrolled' : ''}`}>
      <span className="nav-logo">
        SHOP<span className="nav-logo-thin">SMART</span>
      </span>
...
const HERO_IMG =
  'https://images.unsplash.com/photo-1593642634315-48f5414c3ad9?w=1400&q=85&fit=crop';
    ...
    <section className="hero">
      ...
      <div className="hero-content fade-in">
        ...
        <p className="hero-sub">
          Precision-engineered tools for those who work with intention.
          <br />
      ...
      <div className="card-img-wrap">
        ...
        {product.tag && (
          <span
            ...
            style={{ color: tagColor, borderColor: tagColor + '55' }}
          >
            {product.tag}
          </span>
        )}
...
export default function App() {
  ...
  const newArrivals = products.filter((p) =>
    ['New Season', "Editor's Pick", 'Limited Drop'].includes(p.tag)
  );
  const bestSellers = products.filter((p) => p.tag === 'Best Seller');
  const rest = products.filter(
    (p) => !newArrivals.includes(p) && !bestSellers.includes(p)
  );
      ...
      {!loading && !error && (
        <>
          <Section
            ...
            products={[...newArrivals, ...rest].slice(0, 4)}
            onAdd={addToCart}
          />
